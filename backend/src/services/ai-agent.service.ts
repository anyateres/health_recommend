...
export class AIAgentService {
  ...
  static async getNutritionAdvice(query: string, context?: any): Promise<string> {
    try {
...
Provide practical, evidence-based advice. Keep responses concise but informative.`
  ...
  static async analyzeMealCombination(ingredients: string[]): Promise<{
...
Return JSON with:
{
  ...
  static async getPersonalizedRecommendations(
    userProfile: { age?: number; weight?: number; activity?: string; goals?: string },
    recentAnalysis: any
  ): Promise<string[]> {
    try {
